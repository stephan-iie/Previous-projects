...
// ********************* CHECK RIGHT AND LEFT SENSORS FUNCTIONS

int checkRobotSensorRightAllWalls(struct Robot * robot, struct Wall_collection * head) {
    // intialises the wall, integer i?, directions scores hit and lenght of sensitiviet
    // returns score
    struct Wall_collection *ptr, *head_store;
    int i;
    double xDir, yDir;
    ...
    head_store = head; //walls?
    robotCentreX = robot->x+ROBOT_WIDTH/2;
    robotCentreY = robot->y+ROBOT_HEIGHT/2; //what is this?
    score = 0;
    sensorSensitivityLength =  floor(SENSOR_VISION/5); // what is this?

    for (i = 4; i < 5; i++)
    {
        ptr = head_store;
        xDir = round(robotCentreX+(ROBOT_WIDTH*0.9-2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/4-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH*0.9-2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/4-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle)*PI/180));
      xTL = (int) xDir;
        yTL = (int) yDir;
...
int checkRobotSensorLeftAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
       xDir = round(robotCentreX+(-ROBOT_WIDTH*0.9)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/4-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH*0.9)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/4-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle)*PI/180));
       xTL = (int) xDir;
        yTL = (int) yDir;
...
//    }
    ...
    for (i = 4; i < 5; i++)
    {
        xDir = round(robotCentreX+(ROBOT_WIDTH*0.9-2)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/4-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY+(ROBOT_WIDTH*0.9-2)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/4-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle)*PI/180));
       xTL = (int) xDir;
        yTL = (int) yDir;

        SDL_Rect rect = {xTL, yTL, sensor_sensitivity, 2};
        SDL_SetRenderDrawColor(renderer, 80+(20*(5-i)), 80+(20*(5-i)), 80+(20*(5-i)), 255);// draws the outer sensors first
        SDL_RenderDrawRect(renderer, &rect);
        SDL_RenderFillRect(renderer, &rect);
    }
    ...
    for (i = 4; i < 5; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/4-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/4-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle)*PI/180));
       xTL = (int) xDir;
        yTL = (int) yDir;
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int  right_sensor, int left_sensor) {
  ...
  // wall finding (r==0)===============================================================================
        ...
        // If we are moving and were about t hit a wall near the fornt
        if (((right_sensor > 0) || (left_sensor > 0)) && (robot->currentSpeed > 0)){
          robot->direction = DOWN;
        printf("--------Wall Finding STOP  \n");

            return;
        }

        else if ((right_sensor > 0)  && (robot->currentSpeed ==0)){
          robot->direction = RIGHT ;
        printf("--------Wall Finding RIGHT  \n");
            return;
        }
        // If we have stopped and were about to hit somethign on the left, go right
        else if ((left_sensor > 0) && (robot->currentSpeed == 0)){
             robot->direction = LEFT ;
            printf("--------Wall Finding LEFT \n");
            return;

        }
        // If we have stopped and were about to hit somethign on the right, go left
        else if (( (left_sensor == 0)) && (right_sensor == 0)&& (robot->currentSpeed <4)){
          robot->direction = UP;
        printf("--------Wall Finding UP \n");
            return;
        }
////
