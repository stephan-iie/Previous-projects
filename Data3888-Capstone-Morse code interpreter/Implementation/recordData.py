...
os.chdir('C:/Users/lkada/OneDrive/Desktop/code/epic blinking')
with ser as s:
    while True:
        ...
        data.append(process_byte_data(byte_data))

        print(f'Data from serial: {str(data[0:4])[:-1]} ... ] (length {len(data)})')

        with open("steph_test_Long_Blink_30","w",newline="") as f:
            writer = csv.writer(f)
