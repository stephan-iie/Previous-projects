...
class View():
    ...
    def load_and_render(self, filename, header="header", tailer="tailer", **kwargs):
        ...
        if filename == "friends":  # using bottles tem[late class to iterate through each friend and add them to a list
            body_template = str(template("friends.tpl", **kwargs))
            header_template = self.load_template("header_loggedin")
            tailer_template = self.load_template(tailer)
        elif filename == "logout":  # using bottles tem[late class to iterate through each friend and add them to a list
            body_template = self.load_template(filename)
            header_template = self.load_template("headerr")
            tailer_template = self.load_template(tailer)
        else:
            body_template = self.load_template(filename)
    ...
    def load_template(self, filename):  # reads each section and stakcs on top of each other to build
        '''
            simple_render
